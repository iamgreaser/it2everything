...
class SN76489:
	...
	def precalc(self):
		...
		# This uses the algo that finaldave supplied
...
# NOTE: this will stop running once it reaches a PC of 0x0000.
# TODO?: exact timing?
class Z80:
	def __init__(self, load, sp, mem, rst, acc, sn, mapper):
		self.sn = sn
	...
	def run(self):
		...
		while self.pc:
			...
			if t > 0 and not oprep:
				print "%04X[%02X]: %02X" % (self.pc-1, self.r8[6], opc)
				#time.sleep(0.05)
				t -= 1
					...
					if y & 8:
						if y & 16: # ... do another round!
							...
							if x == 0x40:
								...
								if z == 0: # LDI/D[R]
									if dbank:
										raise Exception("TODO IXY bli LD")
									...
									self.mem[self.r8[5]|(self.r8[4]<<8)] = v
								elif z == 1: # CPI/D[R]
									raise Exception("todo ED %02X" % opc)
								elif z == 2: # INI/D[R]
									# nobody actually knows what the flags do
									# i'll just do the bare minimum
									raise Exception("todo ED %02X" % opc)
								elif z == 3: # OUTI/D | OTI/DR
									# ditto
									if dbank:
										raise Exception("TODO IXY bli OUT")
...
class SGC:
	def __init__(self, fname):
		...
		self.z80 = Z80(self.ad_load, self.ad_stack, fp.read(), self.ad_rst, self.sng_start, self.sn, self.mapper)
		print "initialising"
